...
export class FormBlogComponent implements OnInit, OnDestroy {
  ...
  loadBlog(id: string) {
    this.blogService.getBlogById(id).subscribe({
      next: (blog) => {
        if (blog) {
          // Handle categories - they come as array of objects or empty array
          this.selectedCategories = blog.categories?.map((cat: any) => 
            typeof cat === 'object' ? parseInt(cat._id) : parseInt(cat)
          ) || [];
          
          // Handle tags - they come as array of objects or empty array  
          this.selectedTags = blog.tags?.map((tag: any) => 
            typeof tag === 'object' ? parseInt(tag._id) : parseInt(tag)
          ) || [];
